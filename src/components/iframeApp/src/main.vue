    ...
    <div class="menu_right" :class="{closeMenu:isOpen}">
      <div class="listP">
        ...
        <div class="list "style="margin-top: 70px" >
          <el-menu  router :default-active="defaultActive" @select="handleSelect" class="el-menu-vertical-demo " @open="handleOpen" @close="handleClose" :collapse="isCollapse" collapse-transition="true">
          <span v-for="item in menuListData" :key="item.title" class="lable-menu">
             <el-submenu v-if="item.children&&item.children.length>0" index="">
               <template slot="title">
                 <i :class="item.icon"></i>
...
<script>
  export default {
    ...
    watch:{
      ...
      $route(data) {
        debugger
        if(data.meta[0].type&&data.meta[0].type=='iframe'){
