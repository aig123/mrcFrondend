<template>
  <section>
    <el-form :model="formData.data" label-width="80px" :rules="formData.rules" :ref="formData.name">
      ...
      <el-form-item :label="config.title" v-for="config in formData.title" style="width: 95%;" :prop="config.field">
        ...
        <el-select v-model="formData.data[config.field]" v-if="config.type=='select'" placeholder="config.placeholder" style="width: 100%">
          <el-option :label="item.name" :value="item.id" v-for="item in config.data"></el-option>
        </el-select>
      ...
      <el-form-item>
        <el-button type="primary" @click="search(item.clickFn)" v-for="item in formData.button">{{item.name}}</el-button>
      </el-form-item>
    </el-form>
  </section>
</template>
